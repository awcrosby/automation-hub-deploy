...
class OpenshiftJobTestRunner(BaseTestRunner):
    ...
    # TODO: keep this here but move all other build stuff to container_build.py or openshift_build.py
    @staticmethod
...
class Build(object):
    ...
    def _get_buildconfigs(self):
        """Get buildconfigs associated to project."""
        r = requests.get(
            self.buildconfig_url, headers=self.auth_header, verify=self.ca_path)
        return r.json()
...
class Job(object):
    ...
    def __init__(self, ocp_domain, namespace, session_token, ca_path, image, job_template, logger):
        ...
        self.job_yaml = job_template.format(
            job_name=self.name,
            image=image,
            artifact_download_url='https://galaxy-dev.ansible.com/download/awcrosby-collection_test-1.0.2.tar.gz',
        )
        self.log = logger or default_logger
